...
in_file = sys.argv[1]
#in_file is good format pdb file by modi_gauss_pdb.py
...
def conect_change_atom_num(line, add_num):
    l_s = line.split() 
    atom1_index = int(l_s[1])
    atom2_index = int(l_s[2])
    ...
    new_line = 'CONECT  ' + atom1 + '  ' + atom2
    return new_line

with open(in_file) as f:
#Part_1
#keep the lines before -ch3    
    for i in range(ch3_min_atom - 1):
        line = f.readline()
        print(line, end='')
...
#change the hetatm lines after -ch3
    for i in range(hetatm_total_num - ch3_max_atom):
        line = f.readline()
